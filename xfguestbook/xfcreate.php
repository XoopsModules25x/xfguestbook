...
case "post":
	...
	if ($_POST['uman']!="") redirect_header("index.php",2,'');
	if ($option['opt_url'] == 2 && preg_match("/(http)|(www)/i",$message))
			$msgstop .= _MD_XFGB_URL_DISABLED.'<br /><br />';
	if (!email_exist($email)) $msgstop .= _MD_XFGB_INVALIDMAIL.'<br /><br />';
	if ($email != '' && !checkEmail($email) ) $msgstop .= _MD_XFGB_INVALIDMAIL.'<br /><br />';
	if (!empty($_FILES['photo']['name'])) {
		xfgb_upload();
	} 
	if ( !empty($msgstop) ) {
