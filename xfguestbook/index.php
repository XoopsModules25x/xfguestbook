...
if(isset($_GET['msg_id'])) 		$msg_id = intval($_GET['msg_id']);
elseif(isset($_POST['msg_id'])) $msg_id = intval($_POST['msg_id']);
else $msg_id = 0;
...
global $nbmsg, $xoopsModule, $xoopsUser, $xoopsModuleConfig, $xoopsTpl, $xoopsConfig, $options, $opt,  $xoopsDB;
	...
	foreach( $msg as $onemsg ){
		...
		if (isset($country_name)) $a_msg['local'] = "<a href=\"index.php?op=show_country&param=".$onemsg->getVar('flagdir')."/".$onemsg->getVar('country')."\">".$country_name."</a>";
...
if (substr($op,0,4)== 'show'){
	...
	$xoopsOption['template_main'] = 'xfguestbook_index.html';
